...
4. 如果你要用本文档里的**浏览器授权按钮/其他交互卡片按钮**,还要在同一页把**卡片回调/消息卡片回传交互**也配上:
   - 回调地址先留空,等 Railway 域名出来后再填
   - 加密和 Verification Token 与上面保持一致
   - 回调地址可以直接复用同一个 `/feishu/webhook`
...
2. **事件订阅请求地址**:填上面拿到的 URL
3. **卡片回调/消息卡片回传交互地址**:如果你要使用交互按钮,也填成同一个 URL
4. 点"保存",飞书会发一次 `url_verification` 请求过去验证。**看到"验证成功"才算 OK**

如果验证失败,八成是这两个原因:
- 域名写错(检查有没有 `https://` 和 `/feishu/webhook` 后缀)
- 加密 Encrypt Key 配错(要么飞书后台和环境变量都不填,要么两边一致)

如果**消息能收到,但卡片按钮点了报错**,优先检查:
- 是否只配了 `im.message.receive_v1`,但没有配置卡片回调/消息卡片回传交互地址
- 卡片回调地址是否也指向了同一个 `https://.../feishu/webhook`
...
正常的——长 agent 任务下单次响应可能 30 秒到几分钟。机器人会先回一句"🤔 思考中…",然后边干活边推送进度。
